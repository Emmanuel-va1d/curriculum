...
Any exciting application you build is likely to change over its lifetime. The change could be as simple as toggling a dropdown menu or as complex as fetching data from an API. React provides primitives to manipulate state of your apps, more specifically components, to make them dynamic. In this lesson, we will learn how to use state in React.  
...
Let's take a look on how we can use state in our code.
...
<span id="useState-hook">The `useState` hook is a built-in hook (we'll talk about hooks later) in React that allows you to define state in a functional component. It takes an initial value as a parameter and returns an array with two elements:</span>
...
Great, you can now use state! But, what's actually going on under the hood? 
...
1. Read [State: A Component's Memory from the React docs](https://react.dev/learn/state-a-components-memory).
...
*   <a class="knowledge-check-link" href="#hooks">What are the rules of hooks?</a>
